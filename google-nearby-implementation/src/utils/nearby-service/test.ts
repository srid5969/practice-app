...
import { HexagonNigeriaService } from './nigeria-hexagon.service';
...
export class TestHexagonNigeriaService {
	private resolution: number;
	private isRunning: boolean = false;
	private currentProgress: Progress = { status: 'idle' };
	private hexagonService: HexagonNigeriaService;
	...
	constructor(resolution = HEXAGON_RESOLUTION) {
		...
		this.hexagonService = new HexagonNigeriaService();
	}
	...
	async startCollection(options: CollectionOptions = {}) {
		...
		try {
			...
			const hexagons = this.generateNigeriaHexagons();
			...
			for (let i = 0; i < testCities.length; i++) {
				...
				const center = {
					lat: testCities[i].lat,
					lng: testCities[i].lng,
				};

				logger.info(
					`Fetching places for center: ${JSON.stringify(center)}`,
				);
				...
				logger.info(
					`Fetched ${results.length} places for center: ${JSON.stringify(center)}`,
				);
				...
				if (options.testMode && allResults.length > 50) {
					logger.info(
						'Test mode: stopping collection with sufficient results',
					);
					break;
				}
			}
			...
			const finalResults = {
				...
				metadata: {
					...
					runtime:
						(
							(Date.now() -
								(this.currentProgress.startTime || 0)) /
							1000
						).toFixed(2) + ' seconds',
				},
			};
	...
	private async saveResults(results: {
		...
		const collectionName = 'saloonsv2';
		...
		await batch.commit();
		logger.info(
			`Saved ${results.data.length} places to Firestore collection '${collectionName}'`,
		);
	}
